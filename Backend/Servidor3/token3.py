...
other_port = 8084
my_port = 8082
my_port_socket = 8087  # Porta do socket para comunicação
my_port_socket2 = 8088  # Porta do socket para comunicação
...
def process_purchase(route):
    """Função que processa a compra da passagem"""
    if route in routes_server3 and routes_server3[route]['passagens'] > 0:
        routes_server3[route]['passagens'] -= 1
        return {"success": True, "remaining": routes_server3[route]['passagens']}  # Retorna as passagens restantes
    else:
...
def passar_token():
    ...
    global token
    token = 1  # Passa o token para o próximo servidor
    ...
    while pending_requests:
        ...
        print("-"*30)
        for rota, dados in routes_server3.items():
...
def descobrir_rotas():
    ...
    for rota, passagens in routes_server3.items():
...
def pass_token_periodicamente():
    try:
        url = 'http://localhost:8082/api/passar_token'
    ...
    try:
        ...
        with socket.socket() as s:
            ...
            while True:
                ...
                if data == 3:  # Verifica se o token recebido é o esperado
                    print("Token recebido!")
                    token = 3  # Atualiza o token
                    c.close()
                    ...
                    with app.test_client() as client:
                        client.post('http://localhost:8082/api/passar_token')
